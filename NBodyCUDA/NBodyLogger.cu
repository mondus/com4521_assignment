...
extern const float *dPositionsX;
extern const float *dPositionsY;
extern const nbody *dBodies;
extern const float *dDensities;
...
void renderHistogramToImage()
{
    if(dDensities) {
        if(!Densities) {
            Densities = (float*)malloc(sizeof(float)*__D*__D);
        }
        assert(cudaMemcpy(const_cast<float*>(Densities), dDensities, sizeof(float)*__D*__D, cudaMemcpyDeviceToHost) == cudaSuccess);
    }
    if(Densities) {
...
void renderNBodyToImage()
{
    if (dBodies) {
        if (!Bodies) {
            Bodies = (nbody*)malloc(sizeof(nbody)*__N);
        }
        assert(cudaMemcpy(const_cast<nbody*>(Bodies), dBodies, sizeof(nbody)*__N, cudaMemcpyDeviceToHost) == cudaSuccess);
    } else if (PositionsX && PositionsY) {
        if (!PositionsX) {
            PositionsX = (float*)malloc(sizeof(float)*__N);
        }
        if (!PositionsY) {
            PositionsY = (float*)malloc(sizeof(float)*__N);
        }
        assert(cudaMemcpy(const_cast<float*>(PositionsX), dPositionsX, sizeof(float)*__N, cudaMemcpyDeviceToHost) == cudaSuccess);
        assert(cudaMemcpy(const_cast<float*>(PositionsY), dPositionsY, sizeof(float)*__N, cudaMemcpyDeviceToHost) == cudaSuccess);
    }
    if (Bodies) {
